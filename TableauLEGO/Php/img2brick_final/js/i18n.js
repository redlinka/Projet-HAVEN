...
(function () {
    ...
    const SKIP_TAGS = new Set([
        ...
        'CODE','PRE','KBD','VAR','INPUT','TEXTAREA','SELECT','OPTION',
    ]);
    ...
    async function translatePage(targetLang) {
        ...
        const allTexts = [...new Set([...textMap.keys(), ...extraTexts])]
        ...
        if (toFetch.length > 0) {
            ...
            for (let i = 0; i < toFetch.length; i += BATCH) {
                ...
                try {
                    ...
                    if (!res.ok) {
                        console.error('[i18n] translate.php HTTP', res.status, await res.text());
                        continue;
                    }
    ...
    function buildSwitcher() {
        ...
        btn.innerHTML = `${cur.flag} <span>${cur.label}</span>${chevron}`;
        ...
        LANGS.forEach(lang => {
            ...
            item.innerHTML = `${lang.flag} <span>${lang.label}</span>`;
            ...
            item.addEventListener('click', e => {
                ...
                btn.innerHTML = `${lang.flag} <span>${lang.label}</span>${chevron}`;
    ...
    function injectSwitcher() {
        ...
        if (slot) {
            ...
            fixed.style.cssText = 'position:fixed;bottom:20px;right:20px;z-index:9999;';
            document.body.appendChild(fixed);
            console.warn('[i18n] #lang-switcher-slot introuvable — sélecteur en position fixe.');
        }
    }
    ...
    /* ── API publique ────────────────────────────────────────── */
    window.I18N = {
        setLang:   lang => { localStorage.setItem(LS_KEY, lang); translatePage(lang); },
