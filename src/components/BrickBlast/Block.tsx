...
// ── Constants ────────────────────────────────────────────────────────────────
const UNIT = 100 / 12;         // matches the 9×9 game grid
...
const GAP = 0.3;              // gap between adjacent bricks
...
// ── Block component ───────────────────────────────────────────────────────────
// Renders a multi-cell piece with a Lego-style look (body + stud per cell).
// Drag to move, click (< 200 ms) to rotate 90°.
export const Block = ({
  initialPosition,
  onHover,
  color,
  ...
  onHover: (ref: THREE.Group | null) => void;
  color: string;
  shape: number[][];
}) => {
  ...
  return (
    <group
      ...
      onPointerEnter={() => onHover(groupRef.current)}
...
// ── Scene: 3 random blocks + shared post-processing ──────────────────────────
// EffectComposer is intentionally lifted here (not inside each Block) so that
// Outline/Pixelation/Scanline are applied once for the whole scene.
  ...
  return (
    <>
      ...
      <EffectComposer autoClear={false}>
        ...
        <Scanline opacity={0.2} density={1} />
      </EffectComposer>
    </>
  );
}