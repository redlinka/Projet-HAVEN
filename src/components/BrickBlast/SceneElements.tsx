...
import { useRoomService } from "../../contexts/RoomServiceContext.tsx";
import {
...
export const GameOverScreen = () => {
  ...
  const { connectedUsers, setGameStarted, isAdmin } = useRoom();
  const navigate = useNavigate();
  const roomService = useRoomService();
  const isMultiplayer = connectedUsers.length >= 2;

  const [finalScore, setFinalScore] = useState(score * 5);
  const [opponentScore, setOpponentScore] = useState<number | null>(null);
  const [resultMessage, setResultMessage] = useState<string>("");
  const [hasCalculated, setHasCalculated] = useState(false);
  const hasSentRef = useRef(false);

  // Étape 1: Envoyer score et attendre l'adversaire
  useEffect(() => {
    if (!isGameOver) {
    ...
    if (hasSentRef.current || hasCalculated) return;
    hasSentRef.current = true;

    const gameScore = score * 5;

    if (isMultiplayer) {
      // Envoyer notre score
      roomService.sendGameEndScore(gameScore, "BRICKBLAST", "NORMAL");
      ...
      const handleOpponentScore = (data: {
        ...
        console.log("[BrickBlast] Score adverse:", data.opponentScore);
        ...
        const { winner: finalWinner, loser: finalLoser } =
          calculateScoreTransfer(winner, loser, 0.25);
      ...
      // Solo: pas de calcul, juste finaliser
      setFinalScore(gameScore);
      setHasCalculated(true);
    }
  }, [isGameOver, isMultiplayer, score]);

  // Étape 2: Envoyer le score final au serveur
  useEffect(() => {
    if (!isGameOver || !hasSentRef.current) return;

    // Attendre le calcul en multijoueur
    const readyToSend =
      !isMultiplayer ||
  ...
  }, [isGameOver, finalScore, isMultiplayer, hasCalculated, opponentScore]);
  ...
  useFrame(() => {
    ...
    if (isGameOver) {
      ...
      overlayRef.current.opacity = THREE.MathUtils.lerp(
...
export const TopBarControls = () => {
  ...
  const handleToggleMusic = () => {
    ...
    toggleBGM(
      `${import.meta.env.BASE_URL}sounds/brickblast/background.mp3`,
      0.25,
      nextMutedState,
    );
  };
  ...
  return (
    ...
    <group position={[0, startY, 5]}>
      ...
      <MenuButton
        ...
        imageSrc={
          isMuted
            ? `${import.meta.env.BASE_URL}img/brickblast/music_off.png`
            : `${import.meta.env.BASE_URL}img/brickblast/music_on.png`
        }
        hoverTint="#44ff44"
